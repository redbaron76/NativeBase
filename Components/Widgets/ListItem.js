        ...
        if (!Array.isArray(this.props.children) && !this.inlinePresent() && !this.stackedPresent() && !this.insetPresent()) {
            ...
            if (this.props.iconLeft && !this.props.iconRight) {
                ...
                newChildren.push(React.cloneElement(iconElement[0], {
                    ...this.getChildProps(iconElement[0]),
                    key: 'listItem0'
                }));
                newChildren.push(<View key='listItem1'
            ...
            else if (this.notePresent()) {
                var noteElement = _.remove(childrenArray, function (item) {
                    if (item && item.props.note && item.type == Text) {
                        return true;
                    }
                });
                ...
                newChildren.push(React.cloneElement(noteElement[0], {
                    ...this.getChildProps(noteElement[0]),
                    key: 'listItem1'
                }));
            }
                ...
                newChildren.push(React.cloneElement(badgeElement[0], {
                    ...this.getChildProps(badgeElement[0]),
                    key: 'listItem1'
                }));
            }
            else if (this.props.iconRight && this.props.iconLeft) {
                ...
                newChildren.push(<View key='listItem0' style={{
                    flexDirection: 'row',
                    alignItems: 'center',
                    justifyContent: 'space-between'
                }}>
                ...
                newChildren.push(React.cloneElement(iconElement[1], {
                    ...this.getChildProps(iconElement[1]),
                    key: 'listItem1'
                }));
            }
                ...
                newChildren.push(<View key='listItem1' style={{
                    flexDirection: 'column',
                    paddingLeft: 15,
                    alignSelf: (this.squareThumbs()) ? 'flex-start' : 'center',
                    flex: 1
                }}>
                    {childrenArray.map((child, i) => {
                        return React.cloneElement(child, {...this.getChildProps(child), key: i});
                ...
                newChildren.push(<View key='listItem0' style={{
                    ...
                    <Text
                        style={{color: this.getTheme().inputColorPlaceholder}}>{this.props.children.props.children.props.label}</Text>
                </View>);
                newChildren.push(<View key='listItem1'
                                       style={{flexDirection: 'column', alignSelf: 'center', flex: 2.2}}>
                    {childrenArray.map((child, i) => {
                        return React.cloneElement(child, {
                            ...this.getChildProps(child),
                            key: i,
                            style: {borderWidth: 0}
                        });
                    })}
                ...
                newChildren.push(<View key='listItem1'
                                       style={{flexDirection: 'row', alignSelf: 'stretch', flex: 1, padding: 0}}>
                    {childrenArray.map((child, i) => {
